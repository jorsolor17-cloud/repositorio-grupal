...
function actualizarContador(): void {
  ...
  const items =
    document.querySelectorAll<HTMLSpanElement>(".contador-item");
...
function renderProducto(p: Producto): HTMLLIElement {
  ...
  precio.innerHTML = `<strong>Precio:</strong> ${formatearMoneda(p.precio)}`;

  // Categoría
  const categoria = document.createElement("p");
  categoria.innerHTML = `<strong>Categoría:</strong> `;

  const tag = document.createElement("span");
  tag.className = "categoria-tag";
  tag.textContent = p.categoria;

  categoria.appendChild(tag);

  // Stock
  const stock = document.createElement("p");
  stock.innerHTML = `<strong>Stock:</strong> ${p.stock} unidades`;
