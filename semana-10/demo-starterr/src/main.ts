...
// ---- Datos QUEMADOS para Hito 1 y 2 
...
async function iniciar(): Promise<void> {
  ...
  if (respClientes.estado !== "listo") {
    return mostrarMensaje(
      respClientes.estado === "error"
        ? "Error: " + respClientes.mensaje
        : "Cargando…"
    );
  }
  ...
  if (respProductos.estado !== "listo") {
    return mostrarMensaje(
      respProductos.estado === "error"
        ? "Error: " + respProductos.mensaje
        : "Cargando…"
    );
  }
  ...
  // ── Hito 2: conexión de la lógica real ──
